...
function parseArgs(argv) {
  const out = { query: null, baseDir: null };
...
const { query: searchQuery, baseDir: baseDirArg } = parseArgs(process.argv.slice(2));
if (!baseDirArg && !searchQuery) {
  console.error('Usage: node timestretch.js <query> [--base-dir <dir>]');
  process.exit(1);
}
